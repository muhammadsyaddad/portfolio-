...
const App: React.FC = () => {
  ...
  const syncDateColumn = (sourceElement: HTMLDivElement | null) => {
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    syncDateColumn(e.currentTarget);
  };
  ...
  const handleMouseEnter = (ref: React.RefObject<HTMLDivElement>) => {
    syncDateColumn(ref.current);
  };

  // Shared classes for strict alignment
  const ROW_CLASS = "h-6 mb-2 flex items-center";
  const TEXT_CLASS = "text-sm leading-none tracking-tight truncate cursor-default transition-opacity";
  ...
  const COL_CLASS = "h-full overflow-y-auto no-scrollbar pb-64 pt-4 pointer-events-auto";
  ...
  return (
    <div className="h-screen w-full bg-husky-bg text-husky-text font-mono uppercase overflow-hidden flex flex-col selection:bg-white selection:text-husky-bg">
      ...
      {/* MAIN CONTENT */}
      <main className="flex-grow min-h-0 w-full max-w-[1800px] mx-auto px-6 md:px-12 lg:px-16 relative z-0">
        <div className="h-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-4">
          ...
          <div className="lg:col-span-9 h-full relative">
            ...
            <div className={`lg:hidden ${COL_CLASS}`}>
               <div className="mb-12">
                 <h2 className="text-lg font-bold tracking-wider mb-4 opacity-50">PORTFOLIO</h2>
                 {PROJECT_DATA.map((item, i) => (
                    <div key={i} className="mb-4 text-xs md:text-sm leading-snug tracking-tight">
               ...
               <div className="mb-12">
                 <h2 className="text-lg font-bold tracking-wider mb-4 opacity-50">NOTES</h2>
                 {PROJECT_DATA.map((item, i) => (
                    <div key={i} className="mb-4 text-xs md:text-sm leading-snug tracking-tight">
                        <div className="opacity-90">{item.value}</div>
                    </div>
               ...
               <div className="mb-12">
                 <h2 className="text-lg font-bold tracking-wider mb-4 opacity-50">CANVAS</h2>
                 {PROJECT_DATA.map((item, i) => (
                    <div key={i} className="mb-4 text-xs md:text-sm leading-snug tracking-tight">
                        <div className="opacity-90">{item.value}</div>
                    </div>
            ...
            <div className="hidden lg:grid h-full grid-cols-[140px_1fr_1fr_1fr] gap-6">
              ...
              >
                {PROJECT_DATA.map((item, i) => (
                  <div key={i} className={ROW_CLASS}>
                    <span className={`${TEXT_CLASS} opacity-60`}>{item.label}</span>
                  </div>
                ))}
              </div>
              ...
              >
                {PROJECT_DATA.map((item, i) => (
                  <div key={i} className={ROW_CLASS}>
                    <span className={`${TEXT_CLASS} opacity-90 hover:opacity-100`}>{item.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm leading-snug tracking-tight">
                 {PERSONAL_DETAILS.map((item, index) => (
                   <div key={index} className="flex flex-col">
                     ...
                     <span className="opacity-90 font-bold truncate">{item.value}</span>
                   </div>
                 ))}
              </div>
            </div>
